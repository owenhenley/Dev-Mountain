...
class MessageController {
	...
	func subscribeToNotifications(completion: @escaping (Error?) -> Void) {
		...
		// Create CKQuery Subscription
		let subscription = CKQuerySubscription(recordType: Message.messageTypeKey, predicate:NSPredicate(value: true), options: .firesOnRecordCreation)
		
		let notificationInfo = CKSubscription.NotificationInfo()
		notificationInfo.title = "oooooo!"
		notificationInfo.alertBody = "A new message has been posted to the bulletin board!"
		...
		publicDB.save(subscription) { (_, error) in
			if let error = error {
				print("there was an error saving the subscription: \(error.localizedDescription)")
				completion(error)
				return
			}
			completion(nil)
