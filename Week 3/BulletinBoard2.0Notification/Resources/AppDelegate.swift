...
class AppDelegate: UIResponder, UIApplicationDelegate {
	...
	func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
		UNUserNotificationCenter.current().requestAuthorization(options: [.alert, .sound]) { (userDidAllow, error) in
			if let error = error {
				print("There was an error obtaining push notification authorisation: \(error.localizedDescription)")
				return
			}
	...
	func application(_ application: UIApplication, didFailToRegisterForRemoteNotificationsWithError error: Error) {
		print("error registaring remote notifications: \(error.localizedDescription)")
	}
